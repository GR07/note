...
methodsNeedChange.forEach(methodName => {
  ...
  def(arrayMethods, methodName, function () {
    ...
    // 这里 switch 只是为了 给新增的项 增加响应式处理（push unshift splice）
    switch (methodName) {
      case 'push':
      case 'unshift':
        // arguments 就是 push(传入的参数)
        inserted = args
        break
      case 'splice':
        // 因为 splice 的第三个参数才是需要插入数组的参数
        inserted = args.slice(2)
        break
    }
    // 如果是 以上三个方法
    // 因为这三个方法都是 往数组里面新增项
    // 所以跟对象处理方式同理，也要对新增项做响应式处理
    if (inserted) {
      // 则使用 Observer 的实例方法 observeArray 再去遍历执行 observe
      ob.observeArray(inserted)
    }
    
    console.log(`最终每次执行这7个数组方法都会走到这里`)
    // 返回值
    // 例如：对应着原生数组方法的返回值 let a = [1, 2].pop()
    return result
  }, false)
})
