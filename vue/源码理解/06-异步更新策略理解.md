...
这里就是最终 执行 实例 watcher 时传入的第二个参数回调函数，updateComponent，进入 patch 阶段渲染页面

同时也为了重新收集依赖，类似于模板解析时候 实例化 watcher 一样
...
```js
  get () {
    ...
    try {
      ...
      // Vue.prototype.$mount 阶段会执行 mountComponent 内部给 updateComponent 赋值为 vm._update(vm._render(), hydrating)

      // vm._update(vm._render(), hydrating) 就是进入 patch 阶段渲染页面

      // vm._render() 就是 return 生成 Vnode
