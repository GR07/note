...
<template>
  <div class="page">
    <div class="content-wrap">
      <van-pull-refresh v-model="refreshing" @refresh="pullRefresh">
        <van-list v-model="loading" :finished="finished" offset="100" finished-text="xxx" @load="getList">
          <div v-if="list.length" class="position-list">
            <div class="item-wrap" v-for="(item, index) in list" :key="index" @click="toDetail(item)">
              <position-item :item="item" @handleItemCheck="check(index)"></position-item>
            </div>
          </div>
...
<script>
  ...
  async getList () {
    try {
      let res = await this.http('xxx', params)
      // 列表赋值
      this.list.push(...resAddCheck)
      // 空数据时候禁止自动调用接口
      if (!this.list.length) {
        this.finished = true
        return
      ...
      // 上滑查询文案
      this.finishedText = this.list.length ? '没有更多了' : '暂无数据'
    } catch (err) {
      this.toast(err)
      this.finished = true
    } finally {
